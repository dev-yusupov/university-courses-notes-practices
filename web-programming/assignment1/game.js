const gameState = {
    ...
    platformCardsDrawn: { side: 0, center: 0 },
    switchActive: false,
    roundEnded: false,
    abilities: {},
    abilityUsed: {},
    heavyTrafficStation: null
};
...
function loadGameData() {
    ...
    console.log('Loaded stations:', gameState.stations.length, 'lines:', gameState.lines.length);
    return true;
}
...
function useAbility() {
    ...
    if (gameState.abilityUsed[currentLine.id]) {
        console.log('Ability already used for this line!');
        return;
    }
    ...
    switch (ability) {
        case 'double-draw':
            if (!gameState.currentCard) {
                console.log('Draw a card first!');
                return;
            }
            gameState.abilityUsed[currentLine.id] = 'double-draw-active';
            console.log('Double Draw activated! You can draw 2 segments this turn.');
            break;
        ...
        case 'joker':
            ...
            console.log('Joker activated! Current card is now a Joker.');
            break;
        ...
        case 'switch':
            ...
            gameState.switchActive = true;
            console.log('Switch activated! You can draw from any station on this line.');
            break;
            
        case 'heavy-traffic':
            console.log('Click on a station to mark it for Heavy Traffic (counts as 2 for all scoring).');
            ...
            document.querySelectorAll('.station').forEach(el => {
                ...
                if (lineStations.includes(stationId)) {
                    ...
                    el.addEventListener('click', function selectHeavyTraffic() {
                        ...
                        console.log(`Heavy Traffic set at station ${stationId}! It now counts as 2 stations.`);
                        updateAbilityDisplay();
                        updateScoring();
